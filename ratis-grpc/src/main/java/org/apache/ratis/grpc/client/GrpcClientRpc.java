...
public class GrpcClientRpc extends RaftClientRpcWithProxy<GrpcClientProtocolClient> {
  ...
  public RaftClientReply sendRequest(RaftClientRequest request)
    ...
    if (request instanceof GroupManagementRequest) {
      ...
      final SnapshotManuallyRequestProto proto = ClientProtoUtils.toSnapshotManuallyRequestProto(
          (SnapshotManuallyRequest) request);
      return ClientProtoUtils.toRaftClientReply(proxy.snapshotManually(proto));
    } else {
