...
public class TestGrpcMessageMetrics extends BaseTest
  ...
  static void assertMessageCount(RaftServerImpl server) throws  Exception {
    ...
    Assert.assertTrue(registry.counter(counter_prefix + "_" + "requestVote" + "_completed_total").getCount() > 0);
    Assert.assertTrue(registry.counter(counter_prefix + "_" + "appendEntries" + "_completed_total").getCount() > 0);
  }
}