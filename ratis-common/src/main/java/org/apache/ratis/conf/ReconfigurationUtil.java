...
public class ReconfigurationUtil {
  ...
  getChangedProperties(RaftProperties newConf, RaftProperties oldConf) {
    Map<String, PropertyChange> changes = new HashMap<String, PropertyChange>();

    // iterate over old configuration
    for (Map.Entry<String, String> oldEntry: oldConf.getProperties().entrySet()) {
    ...
    for (Map.Entry<String, String> newEntry: newConf.getProperties().entrySet()) {
